...
namespace PSView
{
	...
	String *CFriendlyName::get_Name()
	{
		return szName;
	}

	DateTime CFriendlyName::get_Date()
	{
		return dtDate;
	}

	bool CFriendlyName::get_Visible()
	{
		return fVisible;
	}
	...
	*/
	int CFriendlyName::CompareTo(Object __gc * obj)
	{
		CFriendlyName __gc *rhs = dynamic_cast<CFriendlyName *>(obj);
	...
	BOOL CALLBACK EnumWindowsProcHelper(HWND hwnd, LPARAM lparam)
	{
		...
		SortedList **data = (SortedList **) lparam;
		DWORD dwProcess = 0;
		...
		if(hThread)
			CloseHandle(hThread);
		hThread = 0;
		CFriendlyName *s = __gc new CFriendlyName(name, date, visible);
		CProcessInfo *p = dynamic_cast<CProcessInfo *>((*data)->get_Item(__box(dwProcess)));
		p->Add(s);
	...
	SortedList * CProcessViewer::GetProcessList(String *IdleProcessName, String *SystemName)
	{
		...
		SortedList * res = __gc new SortedList();
		SC_HANDLE sc = 0;
		...
		if(hSnap == 0 || hSnap == INVALID_HANDLE_VALUE)  // Couldn't snap, use a different method
		{
			...
			{
				res->set_Item(__box(pdwList[i]), GetProcessInfo(pdwList[i], IdleProcessName, SystemName));
			}
		}
		else	// Snap successful, using that way to get information
		{
			...
			if(Process32First(hSnap, &procEntry))	// Go through each of the processes
			{
				do
				{
					...
					CProcessInfo *p = __gc new CProcessInfo(procEntry.th32ProcessID, __gc new String(procEntry.szExeFile));
					if(!p->ID)	// The idle process has the zero id, so save myself some work by special casing it
					{
						p->Add(__gc new CFriendlyName(IdleProcessName));
					}
					else if(p->ID == 4)
					{
						p->Add(__gc new CFriendlyName(SystemName));
					}
					GetProcessFullPath(p);
					res->set_Item(__box(p->ID), p);
				}
...
#pragma warning( pop )
		{
			...
			for(unsigned int i = 0; i < dwReturned; ++i)
			{
				...
				CFriendlyName *s = __gc new CFriendlyName(__gc new String(svcs[i].lpDisplayName));
				if(s->Name->Length == 0)
					continue;
				...
				CProcessInfo * pr = dynamic_cast<CProcessInfo *>(res->get_Item(__box(svcs[i].ServiceStatusProcess.dwProcessId)));
				pr->Add(s);
				pr->IsService = true;
			}
			if(!dwResume)
	...
	void CProcessViewer::GetProcessFullPath( CProcessInfo * p )
	{
		...
		if (NULL != hProcess )
		{
			...
			{	// Uses size in bytes
				...
				{
					p->FullPath = __gc new String(szProcessName);
				}
			}
	...
	CProcessInfo * CProcessViewer::GetProcessInfo( DWORD processID, String *IdleProcessName, String *SystemName )
	{
		...
		String *nm = (processID == 0) ? IdleProcessName : ((processID == 4) ? SystemName : 0);
		String *fp = 0;
		...
		if (NULL != hProcess )
		{
			...
			{	// Uses size in bytes
				...
				{
					nm = __gc new String(szProcessName);
				}
				// Uses size in bytes
				if ( GetModuleFileNameExW( hProcess, hMod, szProcessName, sizeof(szProcessName)))
				{
					fp = __gc new String(szProcessName);
				}
			}
		}

		CProcessInfo *res = __gc new CProcessInfo(processID, nm);
	...
	CProcessInfo::CProcessInfo(DWORD dwProcessID, String *szProcessName)
	{
		...
		szFullPath = 0;
		szUser = 0;
		rgFriendlyNames = __gc new ArrayList();
	}
	...
	String *CProcessInfo::DefaultFriendlyName()
	{
		if(rgFriendlyNames->Count == 0)
			return S"";
		return (dynamic_cast<CFriendlyName *>(rgFriendlyNames->get_Item(0)))->Name;
	}
	...
	DWORD CProcessInfo::get_ID()
	{ 
		return dwID; 
	}

	String *CProcessInfo::get_Name()
	{
		return szName;
	}

	bool CProcessInfo::get_Modifiable()
	{
		return fModifiable;
	}

	void CProcessInfo::set_Modifiable(bool value)
	{
		fModifiable = value;
	}

	bool CProcessInfo::get_IsService()
	{
		return fIsService;
	}

	void CProcessInfo::set_IsService(bool value)
	{
		fIsService = value;
	}

	String * CProcessInfo::get_FullPath()
	{
		return szFullPath;
	}

	void CProcessInfo::set_FullPath(String *value)
	{
		szFullPath = value;
	}

	int CProcessInfo::get_Count()
	{
	...
	{
		return dynamic_cast<CFriendlyName *>(rgFriendlyNames->Item[idx]);
	}
