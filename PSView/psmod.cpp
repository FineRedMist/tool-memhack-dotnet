...
namespace PSView
{
	...
	{
		if(t == __typeof(Byte) || t == __typeof(Char))
		{
			return 1;
		}
		else if(t == __typeof(Int16) || t == __typeof(UInt16))
		{
			return 2;
		}
		else if(t == __typeof(Int32) || t == __typeof(UInt32))
		{
			return 4;
		}
		else if(t == __typeof(Int64) || t == __typeof(UInt64))
		{
			return 8;
		}
	...
	String *CProcessModifier::MemoryBlockInfo(const MEMORY_BASIC_INFORMATION & mbi)
	{
		...
		s = S"Memory: 0x";
		s = s->Concat(s, a.ToString(S"X8"));
		s = s->Concat(s, S" for ");
		a = mbi.RegionSize;
		s = s->Concat(s, a.ToString());
		s = s->Concat(s, S" bytes (");
		switch(mbi.State)
		{
		case MEM_COMMIT:
			s = s->Concat(s, S"commit, ");
			break;
		case MEM_FREE:
			s = s->Concat(s, S"free, ");
			break;
		case MEM_RESERVE:
			s = s->Concat(s, S"reserve, ");
			break;
		default:
			s = s->Concat(s, S"unknown(");
			a = mbi.State;
		...
		if(mbi.State != MEM_FREE)
		{
			...
			{
			case MEM_IMAGE:
				s = s->Concat(s, S"image, ");
				break;
			case MEM_MAPPED:
				s = s->Concat(s, S"mapped, ");
				break;
			case MEM_PRIVATE:
				s = s->Concat(s, S"private, ");
				break;
			default:
				s = s->Concat(s, S"unknown(");
				a = mbi.Type;
		...
		{
			s = s->Concat(s, S"EXECUTE_READWRITE");
		...
		{
			s = s->Concat(s, S"EXECUTE_WRITECOPY");
	...
	UInt64 CProcessModifier::FindFirst(Object *obj, IProgressIndicator *pb)
	{
		...
		ArrayList * regions = __gc new ArrayList();
		BYTE rgBuf[4096];
		UValueTest tst;
		tst.ull = 0;
		DWORD dwCurPos = 0;
		...
		{
			CRegion * reg = dynamic_cast<CRegion *>(regions->get_Item(j));
	...
	CAddressValue *CProcessModifier::get_AddressValues() __gc []
	{
		if(m_srchType == 0)
			return 0;
		...
		ArrayList *al = __gc new ArrayList();
		...
		for(m_addrs->First(); (addr = m_addrs->GetValue()) != RESULT_NOT_FOUND;)
		{
			...
			CAddressValue *val = __gc new CAddressValue();
			val->Address = addr;
		...
		return dynamic_cast<CAddressValue * __gc []>(al->ToArray(__typeof(CAddressValue)));
	}

	UInt32 CProcessModifier::get_Count()
	{
		if(!m_addrs)
			return 0;
